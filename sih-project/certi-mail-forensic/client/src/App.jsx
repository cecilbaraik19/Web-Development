...
export default function App() {
  ...
  const BACKEND_URL =
    import.meta.env.VITE_API_URL || "https://certimail-forensic.onrender.com";
        ...
        <header className="flex justify-between items-center pb-6 border-b border-slate-800">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-1">
              Unified AI Email Threat & Infrastructure Analyzer
            </p>
          </div>
            ...
            <div className="bg-slate-900/90 backdrop-blur-sm p-5 rounded-xl border border-slate-800 hover:border-cyan-800 hover:shadow-glow-cyan transition-all duration-300 flex flex-col gap-4">
              ...
              <div className="flex gap-2">
                <button
                  ...
                  disabled={loading}
                  className="flex-1 bg-cyan-600 hover:bg-cyan-500 hover:shadow-glow-cyan py-2.5 rounded-lg font-medium text-sm transition-all flex items-center justify-center gap-2 disabled:opacity-80"
                >
                  {loading ? (
                    <span className="font-mono text-xs">
                      {SCAN_SEQUENCE[scanLine]}
                    </span>
                  ) : (
                    <>
            ...
            <div className="bg-slate-900/90 backdrop-blur-sm p-5 rounded-xl border border-slate-800 hover:border-cyan-800 transition-all duration-300">
              ...
              <div className="flex flex-col gap-2 max-h-48 overflow-y-auto pr-1">
                {history.map((item) => (
                  <div
                    key={item._id}
                    className="p-2.5 bg-slate-950 rounded-lg border border-slate-800/80 flex justify-between items-center text-xs hover:border-cyan-800/60 transition-colors"
                  >
                    <div>
                      <span className="font-mono text-cyan-400">
                        {item.extractedIp || "N/A"}
                      </span>
          ...
          <div className="lg:col-span-7 flex flex-col gap-6">
            {report ? (
              <>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <h4 className="text-sm font-mono font-bold text-slate-200">
                        {report.campaign_tag || "CLEAN-TRANSMISSION-BASELINE"}
                      </h4>
                    </div>
                  </div>
                ...
                <div
                  className={`p-5 rounded-xl border flex justify-between items-center animate-pulse-glow ${verdictGlow(report.verdict)} ${
                ...
                {report.ml_classification && (
                  <div className="bg-slate-900/90 backdrop-blur-sm border border-slate-800 p-5 rounded-xl hover:border-cyan-800 transition-all">
                    ...
                    <div className="flex items-center justify-between flex-wrap gap-2">
                      <span className="text-sm font-mono text-cyan-400 uppercase">
                        {report.ml_classification.label}
                      </span>
                      <div className="flex gap-3 text-xs text-slate-400">
                        {Object.entries(
                          report.ml_classification.probabilities || {},
                        ).map(([label, pct]) => (
                          <span key={label}>
                            {label}: {pct}%
                          </span>
                ...
                <div className="grid grid-cols-3 gap-4">
                  {Object.entries(report.authentication).map(([key, value]) => (
                    <div
                      key={key}
                      className="bg-slate-900/90 backdrop-blur-sm border border-slate-800 p-4 rounded-xl text-center hover:shadow-glow-cyan transition-all"
                    >
                ...
                <div className="bg-slate-900/90 backdrop-blur-sm border border-slate-800 p-5 rounded-xl flex flex-col justify-between hover:border-cyan-800 transition-all">
                  <div>
                    <h4 className="text-xs text-slate-400 flex items-center gap-1 mb-2">
                      <Server size={14} /> Extracted IP & Domains
                    </h4>
                    ...
                    {report.infrastructure_masking?.is_likely_masked && (
                      <div className="mt-2 text-xs text-amber-400 flex items-center gap-1">
                        <AlertTriangle size={12} /> Likely VPN/Hosting
                        infrastructure detected (
                        {report.infrastructure_masking.matched_signature})
                      </div>
                    )}
                  </div>
                  <div className="mt-4 pt-3 border-t border-slate-800 flex justify-between items-center">
                    <div>
                      <h4 className="text-xs text-slate-400 flex items-center gap-1 mb-1">
                        <MapPin size={14} /> Observed Location
                      </h4>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-xs">
                    <div className="bg-slate-950 p-3 rounded-lg border border-slate-800">
                      ...
                      <span className="font-medium text-slate-300 truncate block">
                        {report.whois_data?.registrar || "N/A"}
                      </span>
                    </div>
                    <div className="bg-slate-950 p-3 rounded-lg border border-slate-800">
                      <span className="text-slate-500 block text-[10px]">
                      ...
                      <span className="font-medium text-slate-300">
                        {report.whois_data?.creation_date || "N/A"}
                      </span>
                    </div>
                    <div className="bg-slate-950 p-3 rounded-lg border border-slate-800">
                      <span className="text-slate-500 block text-[10px]">
                    ...
                    <div className="bg-slate-950 p-3 rounded-lg border border-slate-800">
                      ...
                      <span className="font-medium text-emerald-400">
                        {report.whois_data?.dnssec || "Unknown"}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-slate-900/90 backdrop-blur-sm border border-slate-800 p-5 rounded-xl">
                  <h4 className="text-xs text-slate-400 mb-2">
                    Detected Language Risk Indicators
                  </h4>
                  <ul className="list-disc list-inside text-xs text-slate-300 space-y-1">
                ...
                <div className="bg-slate-900/90 backdrop-blur-sm border border-slate-800 p-4 rounded-xl overflow-hidden h-[500px] flex flex-col">
                  <h4 className="text-xs text-slate-400 flex items-center gap-1 mb-3">
                    <Globe size={14} /> Interactive Transmission Map
                  </h4>
                  <div className="flex-1 w-full rounded-lg overflow-hidden border border-slate-800">
                    <MapContainer
                      center={[
                        report.estimated_geo.lat || 20.5937,
                        report.estimated_geo.lon || 78.9629,
                      ]}
                      zoom={4}
                    ...
                    >
                      ...
                      >
                        <Popup>
                          Origin Node: {report.extracted_ip}
                          <br />
                          Location: {report.estimated_geo.city},{" "}
                          {report.estimated_geo.country}
                        </Popup>
                      </Marker>
                    </MapContainer>
