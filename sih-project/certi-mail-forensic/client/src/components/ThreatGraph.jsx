...
export default function ThreatGraph({ graphData, title = "Threat Relationship Graph" }) {
  ...
  // Collision radius accounts for the actual label text width (not just the
  // node dot), so nodes are pushed apart FAR enough that labels never overlap.
  const estimateCollisionRadius = (node) => {
    const label = node.label || node.id || '';
    const baseRadius = (node.type === 'current_case' || node.type === 'related_case') ? 8 : 6;
    const estimatedLabelWidth = label.length * 6.5; // rough px width at our font size
    return baseRadius + estimatedLabelWidth * 0.55;
  };

  const tuneForces = (ref, nodeCount) => {
    if (!ref.current) return;
    const strength = nodeCount > 12 ? -650 : nodeCount > 6 ? -420 : -220;
    const linkDist = nodeCount > 12 ? 170 : nodeCount > 6 ? 130 : 100;
    ref.current.d3Force('charge', d3.forceManyBody().strength(strength));
    ref.current.d3Force('link').distance(linkDist).strength(0.4);
    ref.current.d3Force('collide', d3.forceCollide().radius(estimateCollisionRadius).strength(1));
  };

  useEffect(() => {
    if (fgRef.current && graphData?.nodes?.length > 0) {
      tuneForces(fgRef, graphData.nodes.length);
      setTimeout(() => fgRef.current.zoomToFit(700, 60), 700);
    }
  }, [graphData]);
  ...
  useEffect(() => {
    if (isFullscreen && fullscreenFgRef.current && graphData?.nodes?.length > 0) {
      ...
      setTimeout(() => fullscreenFgRef.current.zoomToFit(700, 100), 750);
    }
  }, [isFullscreen]);
  ...
  const resetView = () => {
    ...
    if (ref.current) ref.current.zoomToFit(700, isFullscreen ? 100 : 60);
  };

  const zoomBy = (factor) => {
    const ref = isFullscreen ? fullscreenFgRef : fgRef;
    if (ref.current) ref.current.zoom(ref.current.zoom() * factor, 300);
  };
  ...
  // Every label is ALWAYS drawn — no hiding based on zoom. Clarity comes from
  // giving each node enough physical space (via the collision force above),
  // not from hiding information.
  const nodeCanvasObjectFn = (labelScale = 1) => (node, ctx, globalScale) => {
    if (typeof node.x !== 'number' || typeof node.y !== 'number') return;
    const label = node.label || node.id;
    const color = nodeColor(node.type);
    const radius = (node.type === 'current_case' || node.type === 'related_case') ? 7 : 5.5;
    ...
    const fontSize = Math.max((12 * labelScale) / globalScale, 4 * labelScale);
    ctx.font = `600 ${fontSize}px Inter, sans-serif`;
    const textWidth = ctx.measureText(label).width;
    const padX = 5;
    ctx.fillStyle = 'rgba(2, 6, 23, 0.9)';
    ctx.fillRect(node.x + radius + 4, node.y - fontSize / 2 - 2.5, textWidth + padX * 2, fontSize + 5);
    ctx.strokeStyle = `${color}66`;
    ctx.lineWidth = 0.6;
    ctx.strokeRect(node.x + radius + 4, node.y - fontSize / 2 - 2.5, textWidth + padX * 2, fontSize + 5);

    ctx.textAlign = 'left';
    ctx.textBaseline = 'middle';
    ctx.fillStyle = '#f8fafc';
    ctx.fillText(label, node.x + radius + 4 + padX, node.y);
  };
  ...
  const linkCanvasObjectFn = (labelScale = 1) => (link, ctx, globalScale) => {
    ...
    const fontSize = Math.max((9 * labelScale) / globalScale, 3.2 * labelScale);
      ...
      >
        ...
        >
          <ForceGraph2D
            ...
            nodeRelSize={isFullscreen ? 10 : 8}
            nodeCanvasObject={nodeCanvasObjectFn(isFullscreen ? 1.7 : 1.1)}
            linkCanvasObjectMode={() => 'after'}
            linkCanvasObject={linkCanvasObjectFn(isFullscreen ? 1.7 : 1.1)}
            enableNodeDrag={true}
            ...
            nodePointerAreaPaint={(node, color, ctx) => {
              ctx.fillStyle = color;
              ctx.beginPath();
              ctx.arc(node.x, node.y, isFullscreen ? 18 : 14, 0, Math.PI * 2);
              ctx.fill();
            }}
          />
        </div>
